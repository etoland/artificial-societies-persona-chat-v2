...
- **Export a conversation** as a transcript — several clients use this for "1:1 market research interviews" per the brief, and will want to paste findings into a report. *Caveat:* the reference screenshots of the current product show an existing download icon and a "Compare" button next to the Results header, which likely already export something at the survey level. I didn't have access to the live product to confirm what those currently do, so I can't say whether conversation-level export is a genuine gap or a scope that already exists in some form — worth checking with the team before building this rather than assuming it's missing.
